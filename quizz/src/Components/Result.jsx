...
export default function Result() {
  ...
  useEffect(() => {
    const load = async () => {
      setLoading(true);
      try {
        const res = await axios.get(`${RESULTS_URL}/${resultId}`);
        setResult(res.data);
      } catch (err) {
  ...
  const { correct, wrong, skipped, totalQuestions, scored, totalMarks, percent, autoSubmitted, answers, setTitle } = result;
      ...
      <div className="max-w-2xl mx-auto">
        ...
        <div className={`${t.bgCard} border ${t.border} rounded-3xl p-8 shadow-xl text-center mb-6`}>
          ...
          <h1 className={`text-2xl font-black ${t.text} mb-1`}>
            {autoSubmitted ? "Time's Up! Auto Submitted" : "Test Submitted"}
          </h1>
          {setTitle && (
            <p className={`text-sm ${t.textMuted} mb-2`}>{setTitle}</p>
          )}
          ...
          >
            <span className={`text-3xl font-black ${passed ? t.accent : "text-rose-500"}`}>
              {percent}%
            </span>
                  ...
                  >
                    ...
                    <div className="flex items-start gap-2 mb-3">
                      <span className={`shrink-0 w-6 h-6 rounded-md flex items-center justify-center text-xs font-black
                        ${isCorrect ? "bg-emerald-500 text-white" : skipped ? `${t.accentLight} ${t.accentText}` : "bg-rose-500 text-white"}`}
                      >
                        {i + 1}
                      </span>
                    ...
                    <div className="grid grid-cols-2 gap-1.5 mb-2">
                      {["A", "B", "C", "D"].map((letter) => {
                        const isChosen  = a.selectedAnswer === letter;
                        const isCorrectOpt = a.correctAnswer === letter;
                        return (
                          <div
                            key={letter}
                          ...
                          >
                            <span className={`w-5 h-5 rounded-md flex items-center justify-center font-black text-xs shrink-0
                              ${isCorrectOpt ? "bg-emerald-500 text-white" :
                                isChosen ? "bg-rose-400 text-white" :
                                `${t.accentLight} ${t.accentText}`}`}
                            >
                              {letter}
                            </span>
                            <span className="truncate">{a[`option${letter}`]}</span>
                            {isCorrectOpt && <span className="ml-auto">✓</span>}
                            {isChosen && !isCorrectOpt && <span className="ml-auto">✗</span>}
                          </div>
                        );
                      })}
                    </div>
                    ...
                    <div className={`flex gap-4 text-xs mt-1 flex-wrap`}>
                      <span className={skipped ? t.textMuted : isCorrect ? "text-emerald-600 font-bold" : "text-rose-600 font-bold"}>
        ...
        <button
          ...
          className={`w-full mt-6 ${t.accentBg} ${t.accentBgHover} text-white py-3 rounded-xl font-bold text-sm shadow-md transition-all`}
        >
          Back to Categories
        </button>
      </div>
